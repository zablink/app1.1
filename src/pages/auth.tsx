// pages/auth.tsx - Updated with full functionality + typed AuthFormErrors + validation wiring
...
type SocialButtonProps = {
  ...
  provider: string; // display label only
  bgColor: string;
  hoverColor: string;
  textColor?: string;
  loading?: boolean;
...
const ZabLinkAuth = () => {
  ...
  const initialTab: AuthView = (searchParams.get("tab") === "signup" ? "signup" : "signin");
  ...
  // IMPORTANT: error type uses AuthFormErrors (string messages), not Partial<AuthFormData>
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (currentView === 'signup') {
      ...
      // Signin minimal checks
      const signinErrors: AuthFormErrors = {};
      if (!formData.email.trim()) signinErrors.email = 'กรุณากรอกอีเมล';
      else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) signinErrors.email = 'รูปแบบอีเมลไม่ถูกต้อง';
      if (!formData.password) signinErrors.password = 'กรุณากรอกรหัสผ่าน';
            ...
            <div className="mb-6">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  />
                  {/* Fixed: GitHub uses the correct GitHub icon */}
                  <SocialButton
                    icon={Github}
                    provider="GitHub"
                    bgColor="bg-black"
